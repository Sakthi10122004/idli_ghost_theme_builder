      ...
      {!isPostPage && (
        <div className="bg-amber-50 border border-amber-300 text-amber-900 rounded-md p-2.5 flex items-start gap-2">
          ...
          <div className="text-[11px] leading-snug">
            ...
            Related Posts relies on Ghost&apos;s <code className="font-mono text-[10px] bg-amber-100 px-1 py-0.5 rounded">{"{{primary_tag.slug}}"}</code> and <code className="font-mono text-[10px] bg-amber-100 px-1 py-0.5 rounded">{"{{id}}"}</code>. On <strong>{activePage || "this page"}</strong>, it cannot detect the current post tag.
          </div>
        </div>
      )}
